from classification.legency import provider
import os
import numpy as np
import torch
from classification.legency.pointnet_cls import ModelCls
...
def train_one_epoch(model, criterion, optimizer, scheduler):
    ...
    for fn in range(len(TRAIN_FILES)):
        data, label = provider.load_h5(os.path.join('../', TRAIN_FILES[train_file_idxs[fn]]))
...
def eval_one_epoch(model, criterion):
    ...
    for fn in range(len(TEST_FILES)):
        data, label = provider.load_h5(os.path.join('../', TEST_FILES[fn]))
        data = data[:, 0:NUM_POINT, :]  # 从点云数据中取NUM_POINT(default:1024)个点
        data, label, _ = provider.shuffle_data(data, label)
        label = np.squeeze(label)
