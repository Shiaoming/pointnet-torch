from classification.legency.transform_nets import InputTransformNet, FeatureTransformNet
import torch
...
class ModelCls(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(-1, 1024)
        x = F.relu(self.bn6(self.dropout1(self.fc1(x))))
        x = F.relu(self.bn7(self.dropout2(self.fc2(x))))
        x = self.fc3(x)
