...
from sem_seg.dataloader import ShapenetSemSegDataset, ToTensor
from utils.save_load import *
from models.pointnet_semseg import PointNetSemSeg, LossSemSeg
...
parser.add_argument('--base_dir', dest='base_dir',
                    default="../data/", help='project directory ')
parser.add_argument('--datadir', dest='datadir',
                    default="../data/indoor3d_sem_seg_hdf5_data",
                    help='data directory ')
...
parser.add_argument('--save_every_epoch', '--see', default=1, type=int,
                    metavar='SEE', help='save_every_epoch')
...
parser.add_argument('--test_area', '--test_area', default='Area_1_conferenceRoom_1', type=str,
                    metavar='CN', help='the test area')
parser.add_argument('--part_num', '--pn', default=13, type=int,
...
def build_dataloader(base_dir, path, batch_size, test_area):
    logger = logging.getLogger('global')
    logger.info("Build dataloader...")

    all_files = os.path.join(path, "all_files.txt")

    train_dataset = ShapenetSemSegDataset(root_dir=base_dir, list_filename=all_files, train=True,
                                          test_area=test_area, transform=ToTensor())
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=4)

    test_dataset = ShapenetSemSegDataset(root_dir=base_dir, list_filename=all_files, train=False,
                                         test_area=test_area, transform=ToTensor())
...
def main():
    ...
    model = PointNetSemSeg(in_channels=9, point_num=point_num, K=args.part_num)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch % args.save_every_epoch == 1:
...
def train_one_epoch(train_loader, model, optimizer, device, epoch, point_num):
    ...
    for i_batch, sample_batched in enumerate(train_loader):
        ...
        total_seen += sample_batched['points'].shape[0]
        loss_sum += loss.item()
...
def test_one_epoch(test_loader, model, device, epoch, point_num):
    ...
    # seg_total += seg_total_zero_mask
    seg_ave_acc = np.mean(seg_correct / seg_total)
    logger.info('Test Loss: {:.4f} '.format(epoch_loss))
    logger.info('Segmentation Acc: {:.4f}, seg average part acc: {:.4f}'.format(epoch_acc_seg, seg_ave_acc))
